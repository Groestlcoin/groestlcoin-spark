...
const addrQr = (address, type) => qruri(`groestlcoin:${type == 'bech32' ? address.toUpperCase() : address}`)
...
export const deposit = ({ address, type }) => addrQr(address, type).then(qr => ({ funds, obalance, unitf, conf: { expert } }) =>
  ...
  , div('.my-4.text-center', [
      a('.btn.btn-primary.btn-lg.mb-1', { attrs: { href: `groestlcoin:${address}` } }, 'Open wallet')
    , ' '
